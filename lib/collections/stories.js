...
Meteor.methods({
	...
	upvoteStory: function(currentUser, storyId){
		if (!currentUser){
			throw new Meteor.Error(422, 'You must login to upvote.');
		}

		var story = Stories.findOne(storyId);
		var existingVoter = _.contains(story.voters, currentUser);

		//check if already voted
		if (story.voters && existingVoter){
			Stories.update({_id: storyId}, {$inc: {score: -1}, $pull: { voters: currentUser}});
		} else {
			Stories.update({_id: storyId}, {$inc: {score: 1}, $push: { voters: currentUser}});
		}
	}
})
