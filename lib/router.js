...
Router.route('userStories', {
	...
	waitOn: function(){
		return [
			Meteor.subscribe('userStories', this.params.username),
			Meteor.subscribe('userPhoto', this.params.username)
		] 
	},
	action: function(){
...
Router.route('storySingle', {
	path: '/story/:_id',
	waitOn: function(){
		var username = Stories.findOne(this.params._id).authorName;
		console.log(username);
		return [
			Meteor.subscribe('storySingle', this.params._id),
			Meteor.subscribe('userPhoto', username)
		];
	},
	data: function(){
